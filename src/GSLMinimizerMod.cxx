...
// Implementation file for class GSLMinimizerMod

#include "../interface/GSLMinimizerMod.h"

#include "GSLMultiMinimizerMod.h"
...
GSLMinimizerMod::GSLMinimizerMod( const char *  type) :    BasicMinimizer()
{
   ...
   fGSLMultiMin = new GSLMultiMinimizerMod(algo);
...
GSLMinimizerMod::~GSLMinimizerMod () {
...
void GSLMinimizerMod::SetFunction(const ROOT::Math::IMultiGenFunction & func) {
...
unsigned int GSLMinimizerMod::NCalls() const {
...
bool GSLMinimizerMod::Minimize() {
   ...
   if (function == 0) {
      MATH_ERROR_MSG("GSLMinimizerMod::Minimize","Function has not been set");
      return false;
   }
   ...
   if (npar == 0 || npar < NDim()  ) {
      MATH_ERROR_MSGVAL("GSLMinimizerMod::Minimize","Wrong number of parameters",npar);
      return false;
   }
   if (npar > ndim  ) {
      MATH_WARN_MSGVAL("GSLMinimizerMod::Minimize","number of parameters larger than function dimension - ignore extra parameters",npar);
   }
   ...
   if (NFree() == 0) {
      MATH_INFO_MSG("GSLMinimizerMod::Minimize","There are no free parameter - just compute the function value");
   ...
   if (stepSize < eps) {
      MATH_ERROR_MSGVAL("GSLMinimizerMod::Minimize","Step size is too small",stepSize);
      return false;
   }
   ...
   if (debugLevel >=1 ) std::cout <<"Minimize using GSLMinimizerMod " << fGSLMultiMin->Name() << std::endl;
...
//       printf("df = %5f, tolerance = %5f\n",df,tolerance);
   ...
   if (minFound) {
      if (debugLevel >=1 ) {
         std::cout << "GSLMinimizerMod: Minimum Found" << std::endl;
         int pr = std::cout.precision(18);
...
//      std::cout << "Edm   = " << fState.Edm() << std::endl;
      ...
      if (debugLevel >= -1 ) {
         std::cout << "GSLMinimizerMod: Minimization did not converge" << std::endl;
         if (iterFailed) {
...
const double * GSLMinimizerMod::MinGradient() const {
