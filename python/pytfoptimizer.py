...
__all__ = ['FastExternalOptimizerInterface', 'FastpyOptimizerInterface']


class FastExternalOptimizerInterface(object):
  ...
  def minimize(self,
    ...
    loss_invhess_func = self._make_eval_func(self._invhess,
                                          session,
                                          feed_dict, fetches, loss_callback)
    ...
    packed_var_val = self._minimize(
        ...
        invhess_func=loss_invhess_func,
  ...
  def _minimize(self, initial_val, loss_grad_func, invhess_func, step_callback, optimizer_kwargs):
...
class FastpyOptimizerInterface(FastExternalOptimizerInterface):
  ...
  def _minimize(self, initial_val, loss_grad_func, invhess_func, step_callback, optimizer_kwargs):
    ...
    minimize_kwargs = {
        'jac': True,
        'invhess' : invhess_func,
        'callback': step_callback,
    }

    minimize_kwargs.update(optimizer_kwargs)

    from HiggsAnalysis.CombinedLimit import pyminimize  # pylint: disable=g-import-not-at-top
    result = pyminimize.minimize(*minimize_args, **minimize_kwargs)
...
#def _compute_invhessian(tensor, var_list):
    #mat = []
    #for v1 in var_list:
        #temp = []
        #for v2 in var_list:
            ## computing derivative twice, first w.r.t v2 and then w.r.t v1
            #temp.append(tf.gradients(tf.gradients(tensor, v2)[0], v1)[0])
...
def _compute_invhessian(tensor, var_list):
  hess = tf.hessians(tensor, var_list)
  return tf.matrix_inverse(hess[0])
